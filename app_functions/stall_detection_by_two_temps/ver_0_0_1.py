...
def function(
    ...
    if end_rts > start_rts:  # all datafeed have readings with ts > cursor_ts
        ...
        prev_curr_state_dfr = DfReading.objects.filter(datafeed__id=curr_state_df.id, time=start_rts).first()
        ...
        for rts in grid:
            ...
            if temp_inlet is not None and temp_outlet is not None:
                if temp_outlet - temp_inlet > 0.1:
                    update_map["health"] = HealthGrades.ERROR
                    add_to_alarm_payload(alarm_payload, "Temp out > Temp in", {}, rts, "w")
                else:
