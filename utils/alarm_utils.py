...
def add_to_alarm_payload(
    alarm_payload: dict, alarm_name: str, alarm_dict: dict | None, ts: int, key: Literal["e", "w", "i"]
):
    ...
    {
        1734567890123: {"e":{"CPU Error": {"st": "in"}, "Another error": {}}, "w": {"Some warning": None}},
        1734567890345: {...},
        ...
        }
...
def update_alarm_map(
    ...
    the transistions of individual alarms into the alarm log. The variable "alarm_dict" should contain
    the dictionary of errors or warnings for a certain timestamp. This dictionary looks like
    {"error 1 name": {"st": "in"}, "error 2 name ": {} or None, ...} and usually is part of payload coming from 
    different devices and datastreams (also, this mechanism is used for processing alarms generated by 
    application functions).
    If "has_value" is True, then persistent errors (but not warnings) that have status "in"
    ...
    if alarm_dict is not None:
        for alarm_name, ind_alarm_obj in alarm_dict.items():  # ind_alarm_obj can be {"st": "in"} or {}/None
    ...
    for alarm_name, ind_alarm_obj in upd_alarm_map.items():
        ...
        if ind_alarm_obj["persist"]:
            ...
            # non-persisten alarms acquire "out" when there is no such an alarm in 'alarm_dict'
            if ind_alarm_obj["st"] == "in" and (
                alarm_dict is None or alarm_dict.get(alarm_name, "NO_ALARM") == "NO_ALARM"
            ):
                ind_alarm_obj["st"] = "out"
                ind_alarm_obj["lastTransTs"] = ts
