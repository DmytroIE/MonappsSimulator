...
from common.constants import DataAggrTypes, AugmentationPolicy, VariableTypes, NotToUseDfrTypes, settings
...
class DfrCreator:
    ...
    def execute(self) -> None:  # will be wrapped with transaction.atomic
        ...
        if not is_calculated or self.start_rts >= self.end_rts:
            # the second condition is possible because of 'till_now_margin'
            # substraction in 'get_end_rts_by_last_dsr'
            self.is_catching_up = False
            return
    ...
    def calc_end_rts(self) -> bool:
        if self.ds.is_rbe and self.df.is_aug_on and self.df.aug_policy == AugmentationPolicy.TILL_NOW:
            # the augmentation algorithm will create new df readings until this rounded timestamp
            ...
            last_dsr = DsReading.objects.filter(datastream__id=self.ds.pk).order_by("time").last()
            if last_dsr is None:
    ...
    def calc_start_rts(self) -> bool:
        print("--calc_start_rts is starting...")
        ...
        if self.ds.is_rbe and self.df.is_aug_on:
            ...
            if last_ndm_before_start_rts is None or (
                ...
                self.is_nodata_period = False
            print(f"---no_data_period is {self.is_nodata_period}")
            ...
            if first_ndm_after_start_rts is not None and first_dsr_after_start_rts is None:
                print("********---no ds readings after 'start_rts', only nodata markers")
                return False

            if (
                self.is_nodata_period
                ...
                or (
                    ...
                    and first_dsr_after_start_rts is not None
                    and first_ndm_after_start_rts.time < first_dsr_after_start_rts.time
                )
            ):
                if first_dsr_after_start_rts is None:
                    # no fresh ds readings after the last nodata marker
                    # or there are no ds readings at all
                    # (so the LAST augmentation cannot be performed)
                    print("---no fresh ds readings after the last nodata marker")
                    return False
                # shift 'start_rts' closer to the first ds reading to omit 'empty' periods
                self.start_rts = ceil_timestamp(
                    first_dsr_after_start_rts.time - self.df.time_resample, self.df.time_resample
                )
                print(
    ...
    def create_ds_reading_batch(self, batch_size: int) -> bool:
        ...
        if last_dsr_in_batch is not None:
            ...
            if self.ds.is_rbe and self.df.is_aug_on and self.df.aug_policy == AugmentationPolicy.TILL_NOW:
                ...
                # in certain cases 'potential_batch_end_rts' can be greater than 'self.end_rts_by_last_dsr'
        ...
        if var_type == VariableTypes.CONTINUOUS and aggr_type == DataAggrTypes.AVG:
            ...
            if not self.ds.is_totalizer:
                if self.ds.is_rbe and self.df.is_aug_on:
                    ...
                    self.df_reading_map = resample_ds_readings(
                        self.ds_readings, self.df, self.df.time_resample, DataAggrTypes.SUM
                    )
            else:
                if self.ds.is_rbe and self.df.is_aug_on:
                    sorted_dsrs_and_ndms = self.create_sorted_dsrs_and_ndms_list()
                    ...
                    self.df_reading_map = resample_ds_readings(
                        self.ds_readings, self.df, self.df.time_resample, DataAggrTypes.LAST
                    )
                    if self.df.is_rest_on:
                        if self.ds.time_change is None:
                            raise ValueError(
                        ...
                        last_nat_dfr_from_prev_period = (
                            DfReading.objects.filter(datafeed__id=self.df.pk, time__lte=self.start_rts, restored=False)
                            .order_by("time")
                            .last()
                        )
    ...
    def save_df_readings(self):
        ...
        rts_to_start_with_next_time = self.start_rts
        for idx, rts in enumerate(df_reading_rtss):
            if self.df_reading_map[rts].not_to_use is not None:
                if self.df_reading_map[rts].not_to_use == NotToUseDfrTypes.SPLINE_UNCLOSED:
                    ...
                    rts_to_start_with_next_time = rts - self.df.time_resample
                break
        ...
        set_attr_if_cond(rts_to_start_with_next_time, ">", self.df, "ts_to_start_with")
        ...
        set_attr_if_cond(rts_to_start_with_next_time, ">", self.ds, "ts_to_start_with")
    ...
    def create_sorted_dsrs_and_ndms_list(self):
        nodata_markers = list(
            NoDataMarker.objects.filter(
                datastream__id=self.ds.pk, time__gt=self.start_rts, time__lte=self.batch_end_rts
            ).order_by("time")
        )
